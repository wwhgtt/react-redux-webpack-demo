...
const ListHead = React.createClass({
  ...
  render() {
    ...
    let dishLink = '';
    if (orderType === 'TS') {
      dishLink = `http://${location.host}/orderall/selectDish?shopId=${headDetail.shopId}&type=TS`;
    } else if (orderType === 'WM') {
      dishLink = `http://${location.host}/takeaway/selectDish?shopId=${headDetail.shopId}&type=WM`;
